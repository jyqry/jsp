...
(function (e) {
	e.fn.jqte = function (t) {
		...
		e.fn.jqteValGet = function () {
			return e(this).closest("." + u.css).find("." + u.css + "_editor").html();
		};
		e.fn.ThisObjGet = function () 
		{
			return e(this);
		};
		...
		return this.each(function () {
			...
			function q(e, t) {
				...
				if (n && t == false) {
					...
					if (n.is("[style]"))
						n.find("*").attr("style", n.attr("style"))
			...
			function U() {
				if (!p.data("sourceOpened")) {
					...
					if (t) {
						...
						y.val(n).focus();
					g.click(function (t) {
						if (e(t.target).hasClass(u.css + "_linktypetext") || e(t.target).hasClass(u.css + "_linktypearrow"))
							X(true)
					});
			...
			function z(t) {
				...
				if (w.data("linktype") != "2")
					...
					v.find("img").each(function () {
						var t = e(this).prev("a");
						var n = e(this).next("a");
						if (t.length > 0 && t.html() == "")
							t.remove();
						else if (n.length > 0 && n.html() == "")
							n.remove()
					})
				}
				W();
			...
			function V(e) {
				...
				if (t == "1" && (y.val() == "http://" || y.is("[value^=http://]") || !y.is("[value^=mailto]")))
					y.val("mailto:");
				else if (t != "1" && !y.is("[value^=http://]"))
					y.val("http://");
				else
					y.val(e)
			}
			...
			function Z(e) {
				...
				if (t || !e)
					D.data("status", false).slideUp(200);
				else
					D.data("status", true).slideDown(200)
			}
			function et(e) {
			...
			function st(t) {
				for (var n = 0; n < f.length; n++) {
					if (u[f[n].name] && f[n].emphasis && f[n].tag != "")
						it(f[n].tag) ? p.find("." + u.css + "_tool_" + f[n].cls).addClass(m) : e("." + u.css + "_tool_" + f[n].cls).removeClass(m)
				}
			...
			if (!e(this).data("jqte") || e(this).data("jqte") == null || e(this).data("jqte") == "undefined")
				e(this).data("jqte", true);
			else
				e(this).data("jqte", false);
				...
				if (u[f[T].name]) 
				{
					...
					if (f[T].name == "format" && e.isArray(u.formats)) {
						...
						for (var A = 0; A < s.length; A++) {
							if (s[A] != null)
								p.find("." + u.css + "_cpalette").append("<a " + u.css + '-styleval="' + s[A] + '" class="' + u.css + "_color" + '" style="background-color: rgb(' + s[A] + ')" role="gridcell" unselectable></a>');
							else
								p.find("." + u.css + "_cpalette").append('<div class="' + u.css + "_colorSeperator" + '"></div>')
						}
					}
				}
			}
			...
			_.unbind("click").click(function (t) 
			{
				...
				if (e(this).data("command") == "test2")
				{
					CreateImage(e(this).ThisObjGet());
				}
				if (e(this).data("command") == "displaysource" && !p.data("sourceOpened")) {
					...
					if (!p.data("sourceOpened")) {
						if (e(this).data("command") == "linkcreator") {
							...
							if (e(this).data("command") == "formats" && !e(t.target).hasClass(u.css + "_format"))
								Y();
							K("", false);
							if (v.not(":focus"))
								v.focus()
						} else if (e(this).data("command") == "fSize" || e(this).data("command") == "colors") {
							if (e(this).data("command") == "fSize" && !e(t.target).hasClass(u.css + "_fontsize") || e(this).data("command") == "colors" && !e(t.target).hasClass(u.css + "_color"))
			...
			v.bind("keypress keyup keydown drop cut copy paste DOMCharacterDataModified DOMSubtreeModified", function () {
					...
					for (var t = 0; t < f.length; t++) {
						if (u[f[t].name] && e.keyCode == f[t].key.charCodeAt(0)) {
							if (f[t].command != "" && f[t].command != "linkcreator")
								j(f[t].command, null);
							else if (f[t].command == "linkcreator")
								U();
							return false
						}
					}
				...
				Z(false);
				X();
				if (e.isFunction(u.blur))
					u.blur();
				h.removeClass(u.css + "_focused");
				if (e.isArray(u.formats))
					et(u.formats[0][1])
			});
